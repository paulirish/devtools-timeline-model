...
class SandboxedModel {
  ...
  _buildTree(direction, grouping, startTime = 0, endTime = Infinity) {
    ...
    const treeClassName = direction === 'topdown' ? 'CallTreeTimelineTreeView' : direction === 'bottomup' ? 'BottomUpTimelineTreeView' : '___';
  ...
  bottomUpGroupBy(grouping, startTime = 0, endTime = Infinity) {
    ...
    const tree = new Timeline.BottomUpTimelineTreeView();
    tree.setModel(this._performanceModel);
    return tree;
