...
class SandboxedModel {
  ...
  _getGrouper(groupBy) {
    if (typeof groupBy === 'string') {
      groupBy = Timeline.AggregatedTimelineTreeView.GroupBy[groupBy];
  ...
  topDownGroupBy(grouping) {
    ...
    var topDownGrouped = TimelineModel.TimelineProfileTree.buildTopDown(this._timelineModel.mainThreadEvents(),
        filters, /* startTime */ 0, /* endTime */ Infinity, this._getGrouper(grouping));

    // group the model in-place if it has a group setting
    ...
    new TimelineModelTreeView(topDownGrouped, grouping).sortingChanged('total', 'desc');
  ...
  bottomUpGroupBy(grouping) {
    ...
    new TimelineModelTreeView(bottomUpGrouped, grouping).sortingChanged('self', 'desc');
