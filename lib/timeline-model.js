...
class SandboxedModel {
  ...
  topDown(eventIdCallback) {
    ...
    console.log('total events', this._timelineModel.mainThreadEvents().length);
    // console.log('filterz', filters);
    console.log('how many', filters[0]._visibleTypes.size);

    var topDown = TimelineModel.TimelineProfileTree.buildTopDown(this._timelineModel.mainThreadEvents(),
        filters, /* startTime */ 0, /* endTime */ Infinity, eventIdCallback || TimelineModel.TimelineAggregator.eventId);
    return topDown;
    // this._refreshTree(topDown, 'total', 'desc');
  }

  bottomUp() {
    var noGrouping = Timeline.AggregatedTimelineTreeView.GroupBy.None;
    var topDown = this.topDown(this._createGroupingFunction(noGrouping));
  ...
  bottomUpGroupBy(grouping) {
    var groupSetting = Timeline.AggregatedTimelineTreeView.GroupBy[grouping];
    var topDown = this.topDown(this._createGroupingFunction(groupSetting));
    var bottomUpGrouped = TimelineModel.TimelineProfileTree.buildBottomUp(topDown);

    // sort the grouped tree, in-place
    // return this._refreshTree(bottomUpGrouped, 'self', 'desc');
